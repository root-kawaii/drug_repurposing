...
class Config(object):
    ...
    def __init__(self):
        ...
        self.lib.test_link_prediction.argtypes = [ctypes.c_char_p]
        #
        # self.test_flag = False  # It is not used
        self.in_path = None
        self.out_path = None
        self.bern = 0
        ...
        self.opt_method = "SGD"
        self.optimizer = None
        self.test_link_prediction_flag = False
    ...
    def init(self):
        ...
        if self.test_link_prediction_flag:
    ...
    def set_test_link_prediction(self, flag):
        self.test_link_prediction_flag = flag
            ...
            with self.sess.as_default():
                ...
                with tf.compat.v1.variable_scope("model", reuse=None, initializer=initializer):
                    ...
                    if self.optimizer != None:
                        ...
                        self.optimizer = tf.compat.v1.train.GradientDescentOptimizer(self.alpha)
        ...
        with self.graph.as_default():
            with self.sess.as_default():
                ...
                for times in range(self.train_times):
                    ...
                    t_end = time.time()
                    if self.log_on:
                        print('\rEpoch: {}, loss: {}, time: {}'.format(times, loss, (t_end - t_init)), end="")
    ...
    def test(self, output_file_console):
        with self.graph.as_default():
            with self.sess.as_default():
                ...
                if self.test_link_prediction_flag:
                    ...
                    for times in range(total):
                        ...
                        if self.log_on:
                            if times % 100 == 0:
                                print("\rTriple: ", times, " / ", total, end="")
                    print("\nTriple: ", total, " / ", total)
                    self.lib.test_link_prediction(output_file_console.encode('utf-8'))
                if self.test_triple_classification:
    ...
    def predict_head_entity(self, t, r, k):
        r'''This method predicts the top k head entities given tail entity and relation.
    ...
    def predict_tail_entity(self, h, r, k):
        r'''This method predicts the top k tail entities given head entity and relation.
        ...
        res = self.test_step(test_h, test_t, test_r).reshape(-1).argsort()[:k]
        # print(self.test_step(test_h, test_t, test_r).reshape(-1)[:k])
        # print(res)
        return res
